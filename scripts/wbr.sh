...
loop-laptop() {
	while :; do
			...
			%{r}$(weather)$p%{A:/usr/scripts/vol.sh -t &:}%{A:gnome-control-center wifi &:}$(network)$p%{A}%{A:/usr/scripts/vol.sh -t &:}$(sound)$p%{A}$(battery)  %{A:/usr/scripts/popup/calendar.sh d &:}$(clock)%{A}$p$bg"
			sleep ".2s"
		done |\
