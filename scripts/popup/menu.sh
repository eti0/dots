...
n30f -t "items" -x "34" -y "$(expr "$height" - "88")" -c "cd $HOME && urxvtc & killall n30f" "$item/term.png" &
...
n30f -t "items" -x "34" -y "$(expr "$height" - "148")" -c "feh --randomize --bg-fill $HOME/ownCloud/Wallpapers & killall n30f" "$item/random.png" &
